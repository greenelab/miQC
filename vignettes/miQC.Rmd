...
parameter to "gaussian", as it runs a one-dimensional gaussian mixture model.

```{r}
model3 <- mixtureModel(sce, model_type = "gaussian")
...
```{r}
plotFiltering(sce, model3, posterior_cutoff = 0.9)
```
...
model <- mixtureModel(sce)
plotFiltering(sce, model, posterior_cutoff = 0.6, preserve_lower_bound = FALSE,
                enforce_left_bound = FALSE)
```
...
When the _preserve_lower_bound_ parameter is set to True, any cells below the 
intact cell line are kept:

```{r}
plotFiltering(sce, model, posterior_cutoff = 0.66, preserve_lower_bound = TRUE,
                enforce_left_bound = FALSE)
```
...
_enforce_left_bound_ will identify the cell marked for removal with the lowest
...
from the _scRNAseq_ package, mouse data from Lun et al [@lun_assessing_2017].
Note that this dataset uses ensembl IDs, so we will use biomaRt to determine
which genes are mitochondrial and calculate mitochondrial percentage.
